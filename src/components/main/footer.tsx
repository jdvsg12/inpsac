...
const Footer = () => {
    return (
        <footer className="h-full bg-backgroundGreen px-4 lg:px-16 xl:px-28">
            <div className="grid grid-cols-1 md:grid-cols-2 items-center py-4">
                ...
                <ul className="flex flex-row list-none mt-4 gap-8 [&_li]:md:text-xl mr-0 ml-auto max-md:mx-auto max-md:my-6">
                    {links.map(({ label, href }) => (
                        <li key={label}><Link href={href} className="text-secondary-foreground hover:text-secondary/70">{label}</Link></li>
                    ))}
                </ul>
            </div>
            <div className="flex flex-col-reverse md:flex-row justify-center md:justify-between py-4 items-center border-white border-t">
                <p className="text-secondary-foreground text-2xl font-light py-4">
                    &copy; 2024 Psicología y Cultura
                </p>
                <div className="flex flex-row gap-8 justify-center items-center lg:mr-0 lg:ml-auto">
